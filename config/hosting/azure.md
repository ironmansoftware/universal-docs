...
Finally, we need to configure PowerShell Universal to store data on our file share. On the Application settings tab, set the `Data__ConnectionString` and `Data__RepositoryPath` to store data on the file share. You will need to ensure that you enable the SQLite plugin with `Plugins__0` set to `SQLite`.&#x20;



| Key                      | Value                         | Description                                                                                |
| ------------------------ | ----------------------------- | ------------------------------------------------------------------------------------------ |
| `Data__ConnectionString` | Data Source=/Data/database.db | The database location within the file store.                                               |
| `Data__RepositoryPath`   | /Data/Repository              | The path to the repository directory to save configuration settings within the file store. |
| `Plugins__0`             | SQLite                        | Enables the SQLite persistence plugin in the app.                                          |
